 ...
 *
 */
class Exclude implements TypeNodeResolverExtension, TypeNodeResolverAwareExtension {
	/**
		...
		foreach ( $constantArrays as $constantArray ) {
			...
			foreach ( $constantArray->getKeyTypes() as $i => $keyType ) {
				if ( $keysType->isSuperTypeOf( $keyType )->yes() ) {
					// Eliminate keys that are in the Exclude type
					continue;
				}
